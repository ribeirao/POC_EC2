...
public class ReflectionHelper {
    ...
    public static String generateMethodArgumentsDescription(JoinPoint joinPoint)
        ...
        for (int i = 0; i < params.length; i++) {
            StringBuilder paramDesc =
                    generateParamDescription(parameterTypes[i], params[i], paramAnnotations[i]);
    ...
    private static StringBuilder generateParamDescription(Class<?> parameterType, Object value,
        ...
        for (Annotation annotation : annotations) {
            if (annotation instanceof NotLoggable)
                canBeLogged = false;
        }
